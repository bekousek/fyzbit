...
**První běh workshopu (11. 9. 2026)** prošel se šesti účastníky a fungovalo všechno kromě **tlakoměru**: chvíli měřil, pak dlouho nic, a občas problikla hláška `no HX711 on P0/P1`. Všem šesti stejně, na kabelech, které jinde fungovaly, a RESET nepomáhal — tedy ne kabely. Příčinou byl **timeout 150 ms** v `HX_READY_TIMEOUT_MS` (viz níže); tentýž mechanismus stál i za tím občasným problikáváním u siloměru, které jsme předtím mylně přisoudili nedoléhajícímu vodiči.
...
- **Timeout na převodník musí pokrýt ustálení, ne jen periodu.** `HX_READY_TIMEOUT_MS` bylo 150 ms proti nominální periodě HX710B 100 ms, což vypadá jako rezerva — jenže čip, který se právě resetoval nebo se probudil z power-downu (stačí 60 µs s SCK nahoře, a bitbangované hodiny z TypeScriptu se tomu prahu blíží), musí nejdřív **ustálit filtr, při 10 Hz řádově 400 ms**. Každé takové zakolísání se proto hlásilo jako „modul tam není" a stálo půl sekundy dat: chvíli měří, pak dlouho nic. Teď je timeout **600 ms**. Rychlejší čidla to nezdrží — celý timeout vyčerpá jen skutečně chybějící modul.
- **Pulzy za daty nejsou u HX710B zesílení, ale volba režimu.** Driver je psaný pro HX711, takže pulzy následující za 24 datovými bity považuje za volbu kanálu a zesílení. HX710B jimi vybírá výstupní režim: **25 pulzů = diferenciální vstup 10 Hz, 27 pulzů = tentýž vstup 40 Hz** (26 je teplota/VDD). Zesílení má pevné, takže `pressScale` to nemění. Tlakoměr proto jede na `HX_GAIN_HX710B = 64` (driver z toho udělá 27 pulzů) — čtyřikrát rychlejší konverze a **ustálení ~100 ms místo ~400 ms**, což je celý důvod, proč se to vejde do rozumného timeoutu. Siloměr zůstává na 128, tam je to skutečné zesílení PGA. Kdyby se u tlakoměru někdy rozjelo měřítko, tohle je první místo, kam se podívat.
