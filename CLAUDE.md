...
- **Firmware smí sáhnout jen na P0, P1 a P2.** Jen ty (plus 3V a GND) mají velký pad s dírou pro krokosvorku; P3–P16 jsou milimetrové proužky, na které se bez breakout desky nedá připojit nic — a školní sada ji mít nemusí. Siloměr proto nesedí na P15/P16, kam ho dává rozšíření `fyzikalni_senzory`, ale na P0/P1 společně s tlakoměrem: uvnitř obou modulů je stejný převodník HX711 na stejném páru drátů, takže sedí na stejných padech (barvy vodičů ale společné nemají, viz další bod). Že si senzory piny přebírají, nevadí — připojený je vždycky jen jeden. V `WiringDiagram.ts` to hlídá typ `Pad`, který je odvozený z tabulky pěti padů.
- **Barva vodiče ve schématu patří pozici na liště, ne jménu signálu.** Propojovací kabel je jedna čtyřpinová dutinka Dupont zapojená černá–žlutá–modrá–červená, takže žlutá krokosvorka dostane to, co modul natiskl na druhou pozici od GND: u siloměru DT, u sonaru Echo, ale u tlakoměru SCK. Rozdat barvy podle signálu („hodiny modrá, data žlutá" u obou převodníků) vypadalo logicky a přesně to na prvním workshopu prohodilo tlakoměru data a hodiny. Teploměr má kabel vlastní, datová žíla je žlutá. V `WiringDiagram.ts` má proto každý signálový vodič barvu napsanou explicitně a záložní `SIGNAL_FALLBACK` se u žádného ze čtyř senzorů nepoužije.
